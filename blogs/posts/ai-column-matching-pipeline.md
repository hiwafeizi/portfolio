...
The architecture for this came from stepping away from the screen. I went for a walk, thought through the problem, and then drew a draft on the whiteboard. By the time I sat down to code I already had the full plan in my head.
...
For each SAS column, compare its 100 values against every Snowflake column's 100 values. If all 100 values match (accounting for type differences, rounding, and encoding), that's a confirmed match. If not, the column is not listed as matched. No percentages, no "probably." Either it's a 100% match or it's not in the results.

This is where the 100 row sample paid off. With enough rows, false positives become essentially impossible. Two columns might accidentally match on 5 rows, but they won't match on all 100 unless they're actually the same data.

### Step 7: Output

The pipeline generated an Excel file for each table containing:

**Matched columns.** SAS column, Snowflake column, and the data from both sides showing the match.
...
On average about **93% of columns per table were confirmed matches**. Several tables hit 100%. The remaining columns that couldn't be matched weren't pipeline errors. They were upstream data quality issues. Values that were supposed to be the same across systems but weren't. Columns where the data had been transformed in ways that weren't documented anywhere.

The pipeline surfaced these issues correctly. When it couldn't match a column with 100% certainty, it simply didn't list it as matched. That's exactly what you want. The tool should tell you where the problems are, not hide them behind a confidence score.

The whole thing was built in **less than two weeks**. Most of the code was written with Claude. The architecture design, the walk and the whiteboard session, took about a day. The rest was implementation and iteration.
...
### The unmatched 7% was more interesting than the matched 93%

If the pipeline had matched every single column, I would have been suspicious. Real data has real problems. The fact that 7% of columns couldn't be matched, and that this turned out to be genuine data quality issues, actually validated the approach.

A tool that always says "everything is fine" is useless. A tool that says "here's what matches, here's what doesn't, and here's the evidence" is useful. The flagged mismatches led to investigations that uncovered real upstream issues that would have caused problems later in the migration anyway.
